...
class TrafficStateDataset(AbstractDataset):
    ...
    def _load_rel(self):
        ...
        """
        relfile = np.load(self.data_path + self.rel_file + '.adj')
        afc_mx = np.load(self.data_path + self.rel_file + '.afc')
        adj_mx1 = np.load(self.data_path + self.rel_file + '.adj1')
        self.adj_mx = relfile
        self.adj_mx1 = adj_mx1
        self.afc_mx = afc_mx
        self.coarse_nodes = len(adj_mx1)
        self._logger.info('load adj_mx1: {}'.format(self.adj_mx1.shape))
        self._logger.info('load afc_mx: {}'.format(self.afc_mx.shape))
        self._logger.info("Loaded file " + self.rel_file + '.adj, shape=' + str(self.adj_mx.shape))
    ...
    def _load_dyna_3d(self, filename):
        ...
        # dynafile = dynafile.replace(0, np.nan)# 把0替换成nan
        # dynafile = dynafile.dropna(how='all', axis=0)# 删除所有为nan的行
        # dynafile = dynafile.replace(np.nan, 0)# 再把nan替换成0  
