...
class TransTrafficStateDataset(AbstractDataset):
    """
    交通状态预测数据集的基类。
    ...
    对于Source和Target会有两组 [X_s,Y_s] & [X_t,Y_t]
    """

    def __init__(self, config):
        self.config = config
        self.source_dataset = self.config.get('source_dataset', '')
        self.target_dataset = self.config.get('target_datasset', 'PEMSD8')
        ...
        self.parameters_str = \
            str(self.source_dataset) + '_' + str(self.input_window) + '_' + str(self.output_window) + '_' \
        ...
        self.tgt_parameters_str = \
            str(self.source_dataset) + '_' + str(self.input_window) + '_' + str(self.output_window) + '_' \
            + str(self.train_rate) + '_' + str(self.eval_rate) + '_' + str(self.scaler_type) + '_' \
            + str(self.batch_size) + '_' + str(self.load_external) + '_' + str(self.add_time_in_day) + '_' \
            + str(self.add_day_in_week) + '_' + str(self.pad_with_last_sample)
        self.tgt_cache_file_name = os.path.join('./libcity/cache/dataset_cache/',
                                            'traffic_state_{}.npz'.format(self.tgt_parameters_str))
        self.cache_file_folder = './libcity/cache/dataset_cache/'
        ensure_dir(self.cache_file_folder)
        # Source dataset
        self.source_data_path = './raw_data/' + self.source_dataset + '/'
        if not os.path.exists(self.source_data_path):
            raise ValueError("Dataset {} not exist! Please ensure the path "
                             "'./raw_data/{}/' exist!".format(self.source_dataset, self.source_dataset))
        # Target dataset
        self.target_data_path = './raw_data/' + self.target_dataset + '/'
        if not os.path.exists(self.target_data_path):
            raise ValueError("Dataset {} not exist! Please ensure the path "
                             "'./raw_data/{}/' exist!".format(self.target_dataset, self.target_dataset))
        # 加载数据集的config.json文件
        self.source_weight_col = self.config.get('source_weight_col', '')
        self.source_data_col = self.config.get('source_data_col', '')
        self.source_ext_col = self.config.get('source_ext_col', '')
        # Target
        self.target_weight_col = self.config.get('target_weight_col', '')
        self.target_data_col = self.config.get('target_data_col', '')
        self.target_ext_col = self.config.get('target_ext_col', '')
        # Source
        self.source_geo_file = self.config.get('source_geo_file', self.source_dataset)
        self.source_rel_file = self.config.get('source_rel_file', self.source_dataset)
        self.source_data_files = self.config.get('source_data_files', self.source_dataset)
        self.source_ext_file = self.config.get('source_ext_file', self.source_dataset)
        # Target
        self.target_geo_file = self.config.get('target_geo_file', self.target_dataset)
        self.target_rel_file = self.config.get('target_rel_file', self.target_dataset)
        self.target_data_files = self.config.get('target_data_files', self.target_dataset)
        self.target_ext_file = self.config.get('target_ext_file', self.target_dataset)
        ...
        self.source_adj_mx = None
        self.target_adj_mx = None
        self.scaler = None
        self.ext_scaler = None
        self.feature_dim = 0
        self.ext_dim = 0
        self.source_num_nodes = 0
        self.target_num_nodes = 0
        self.num_batches = 0
        self._logger = getLogger()
        if os.path.exists(self.source_data_path + self.source_geo_file + '.geo'):
            self._load_source_geo()
            self._load_target_geo()
        else:
            raise ValueError('Not found .geo file!')
        if os.path.exists(self.source_data_path + self.source_rel_file + '.rel'):  # .rel file is not necessary
            self._load_source_rel()
            self._load_target_rel()
        else:
            self.source_adj_mx = np.zeros((len(self.geo_ids), len(self.geo_ids)), dtype=np.float32)

    def _load_source_geo(self):
        """
        加载.geo文件，格式[geo_id, type, coordinates, properties(若干列)]
        """
        source_geofile = pd.read_csv(self.source_data_path + self.source_geo_file + '.geo')
        self.source_geo_ids = list(source_geofile['geo_id'])
        self.source_num_nodes = len(self.source_geo_ids)
        self.source_geo_to_ind = {}
        self.source_ind_to_geo = {}
        for index, idx in enumerate(self.source_geo_ids):
            self.source_geo_to_ind[idx] = index
            self.source_ind_to_geo[index] = idx
        self._logger.info("Loaded file " + self.source_geo_file + '.geo' + ', num_nodes=' + str(len(self.source_geo_ids)))

    def _load_target_geo(self):
        """
        加载.geo文件，格式[geo_id, type, coordinates, properties(若干列)]
        """
        target_geofile = pd.read_csv(self.target_data_path + self.target_geo_file + '.geo')
        self.target_geo_ids = list(target_geofile['geo_id'])
        self.target_num_nodes = len(self.target_geo_ids)
        self.target_geo_to_ind = {}
        self.target_ind_to_geo = {}
        for index, idx in enumerate(self.target_geo_ids):
            self.target_geo_to_ind[idx] = index
            self.target_ind_to_geo[index] = idx
        self._logger.info("Loaded file " + self.target_geo_file + '.geo' + ', num_nodes=' + str(len(self.target_geo_ids)))

    def _load_source_rel(self):
        """
        ...
        Returns:
            np.ndarray: self.source_adj_mx, N*N的邻接矩阵
        """
        source_relfile = pd.read_csv(self.source_data_path + self.source_rel_file + '.rel')
        self.source_afc_mx = np.load(self.source_data_path + self.source_rel_file + '.afc')
        ...
        if self.source_weight_col != '':  # 根据weight_col确认权重列
            if isinstance(self.source_weight_col, list):
                if len(self.source_weight_col) != 1:
                    raise ValueError('`weight_col` parameter must be only one column!')
                self.source_weight_col = self.source_weight_col[0]
            self.source_distance_df = source_relfile[~source_relfile[self.source_weight_col].isna()][[
                'origin_id', 'destination_id', self.source_weight_col]]
        else:
            if len(source_relfile.columns) > 5 or len(source_relfile.columns) < 4:  # properties不只一列，且未指定weight_col，报错
                raise ValueError("Don't know which column to be loaded! Please set `weight_col` parameter!")
            elif len(source_relfile.columns) == 4:  # 4列说明没有properties列，那就是rel文件中有的代表相邻，否则不相邻
                self.calculate_weight_adj = False
                self.set_weight_link_or_dist = 'link'
                self.init_weight_inf_or_zero = 'zero'
                self.source_distance_df = source_relfile[['origin_id', 'destination_id']]
            else:  # len(relfile.columns) == 5, properties只有一列，那就默认这一列是权重列
                self.source_weight_col = source_relfile.columns[-1]
                self.source_distance_df = source_relfile[~source_relfile[self.source_weight_col].isna()][[
                    'origin_id', 'destination_id', self.source_weight_col]]
        # 把数据转换成矩阵的形式
        self.source_adj_mx = np.zeros((len(self.source_geo_ids), len(self.source_geo_ids)), dtype=np.float32)
        if self.init_weight_inf_or_zero.lower() == 'inf' and self.set_weight_link_or_dist.lower() != 'link':
            self.source_adj_mx[:] = np.inf
        for row in self.source_distance_df.values:
            if row[0] not in self.source_geo_to_ind or row[1] not in self.source_geo_to_ind:
                continue
            if self.set_weight_link_or_dist.lower() == 'dist':  # 保留原始的距离数值
                self.source_adj_mx[self.source_geo_to_ind[row[0]], self.source_geo_to_ind[row[1]]] = row[2]
                if self.bidir_adj_mx:
                    self.source_adj_mx[self.source_geo_to_ind[row[1]], self.source_geo_to_ind[row[0]]] = row[2]
            else:  # self.set_weight_link_or_dist.lower()=='link' 只保留01的邻接性
                self.source_adj_mx[self.source_geo_to_ind[row[0]], self.source_geo_to_ind[row[1]]] = 1
                if self.bidir_adj_mx:
                    self.source_adj_mx[self.source_geo_to_ind[row[1]], self.source_geo_to_ind[row[0]]] = 1
        self._logger.info("Loaded file " + self.source_rel_file + '.rel, shape=' + str(self.source_adj_mx.shape))
        # 计算权重
        if self.distance_inverse and self.set_weight_link_or_dist.lower() != 'link':
            self._source_distance_inverse()
        elif self.calculate_weight_adj and self.set_weight_link_or_dist.lower() != 'link':
            self._calculate_source_adjacency_matrix()

    def _load_target_rel(self):
        target_relfile = pd.read_csv(self.target_data_path + self.target_rel_file + '.rel')
        self.target_afc_mx = np.load(self.target_data_path + self.target_rel_file + '.afc')
        self._logger.info('set_weight_link_or_dist: {}'.format(self.set_weight_link_or_dist))
        self._logger.info('init_weight_inf_or_zero: {}'.format(self.init_weight_inf_or_zero))
        if self.target_weight_col != '':  # 根据weight_col确认权重列
            if isinstance(self.target_weight_col, list):
                if len(self.target_weight_col) != 1:
                    raise ValueError('`weight_col` parameter must be only one column!')
                self.target_weight_col = self.target_weight_col[0]
            self.target_distance_df = target_relfile[~target_relfile[self.target_weight_col].isna()][[
                'origin_id', 'destination_id', self.target_weight_col]]
        else:
            if len(target_relfile.columns) > 5 or len(target_relfile.columns) < 4:  # properties不只一列，且未指定weight_col，报错
                raise ValueError("Don't know which column to be loaded! Please set `weight_col` parameter!")
            elif len(target_relfile.columns) == 4:  # 4列说明没有properties列，那就是rel文件中有的代表相邻，否则不相邻
                self.calculate_weight_adj = False
                self.set_weight_link_or_dist = 'link'
                self.init_weight_inf_or_zero = 'zero'
                self.target_distance_df = target_relfile[['origin_id', 'destination_id']]
            else:  # len(relfile.columns) == 5, properties只有一列，那就默认这一列是权重列
                self.target_weight_col = target_relfile.columns[-1]
                self.target_distance_df = target_relfile[~target_relfile[self.target_weight_col].isna()][[
                    'origin_id', 'destination_id', self.target_weight_col]]
        # 把数据转换成矩阵的形式
        self.target_adj_mx = np.zeros((len(self.target_geo_ids), len(self.target_geo_ids)), dtype=np.float32)
        if self.init_weight_inf_or_zero.lower() == 'inf' and self.set_weight_link_or_dist.lower() != 'link':
            self.target_adj_mx[:] = np.inf
        for row in self.target_distance_df.values:
            if row[0] not in self.target_geo_to_ind or row[1] not in self.target_geo_to_ind:
                continue
            if self.set_weight_link_or_dist.lower() == 'dist':  # 保留原始的距离数值
                self.target_adj_mx[self.target_geo_to_ind[row[0]], self.target_geo_to_ind[row[1]]] = row[2]
                if self.bidir_adj_mx:
                    self.target_adj_mx[self.target_geo_to_ind[row[1]], self.target_geo_to_ind[row[0]]] = row[2]
            else:  # self.set_weight_link_or_dist.lower()=='link' 只保留01的邻接性
                self.target_adj_mx[self.target_geo_to_ind[row[0]], self.target_geo_to_ind[row[1]]] = 1
                if self.bidir_adj_mx:
                    self.target_adj_mx[self.target_geo_to_ind[row[1]], self.target_geo_to_ind[row[0]]] = 1
        self._logger.info("Loaded file " + self.target_rel_file + '.rel, shape=' + str(self.target_adj_mx.shape))
        # 计算权重
        if self.distance_inverse and self.set_weight_link_or_dist.lower() != 'link':
            self._target_distance_inverse()
        elif self.calculate_weight_adj and self.set_weight_link_or_dist.lower() != 'link':
            self._calculate_target_adjacency_matrix()
    ...
    def _calculate_source_adjacency_matrix(self):
        """
        ...
        Returns:
            np.ndarray: self.source_adj_mx, N*N的邻接矩阵
        """
        self._logger.info("Source Start Calculate the weight by Gauss kernel!")
        distances = self.source_adj_mx[~np.isinf(self.source_adj_mx)].flatten()
        std = distances.std()
        self.source_adj_mx = np.exp(-np.square(self.source_adj_mx / std))
        self.source_adj_mx[self.source_adj_mx < self.weight_adj_epsilon] = 0

    def _calculate_target_adjacency_matrix(self):
        """
        使用带有阈值的高斯核计算邻接矩阵的权重，如果有其他的计算方法，可以覆盖这个函数,
        公式为：$ w_{ij} = \exp \left(- \\frac{d_{ij}^{2}}{\sigma^{2}} \\right) $, $\sigma$ 是方差,
        小于阈值`weight_adj_epsilon`的值设为0：$  w_{ij}[w_{ij}<\epsilon]=0 $
        ...
        Returns:
            np.ndarray: self.target_adj_mx, N*N的邻接矩阵
        """
        self._logger.info("Source Start Calculate the weight by Gauss kernel!")
        distances = self.target_adj_mx[~np.isinf(self.target_adj_mx)].flatten()
        std = distances.std()
        self.target_adj_mx = np.exp(-np.square(self.target_adj_mx / std))
        self.target_adj_mx[self.target_adj_mx < self.weight_adj_epsilon] = 0

    def _source_distance_inverse(self):
        self._logger.info("Start Calculate the weight by _distance_inverse!")
        self.source_adj_mx = 1 / self.source_adj_mx
        self.source_adj_mx[np.isinf(self.source_adj_mx)] = 1

    def _target_distance_inverse(self):
        self._logger.info("Start Calculate the weight by _distance_inverse!")
        self.target_adj_mx = 1 / self.target_adj_mx
        self.target_adj_mx[np.isinf(self.target_adj_mx)] = 1

    def _load_dyna(self, filename,source):
        """
    ...
    def _load_dyna_3d(self, filename ,source):
        """
        ...
        if source == True:
            dynafile = pd.read_csv(self.source_data_path + filename + '.dyna')
            self.geo_ids = self.source_geo_ids
        else:
            dynafile = pd.read_csv(self.target_data_path + filename + '.dyna')
            self.geo_ids = self.target_geo_ids
        if self.target_data_col == self.source_data_col:
            self.data_col = self.source_data_col
    ...
    def _generate_data(self):
        ...
        # 处理多数据文件问题
        # Source
        if isinstance(self.source_data_files, list):
            source_data_files = self.source_data_files.copy()
        else:  # str
            source_data_files = [self.source_data_files].copy()
        # 加载外部数据
        if self.load_external and os.path.exists(self.source_data_path + self.source_ext_file + '.ext'):  # 外部数据集
            ext_data = self._load_ext()
        else:
            ext_data = None
        x_s_list, y_s_list = [], []
        for filename in source_data_files:
            df = self._load_dyna(filename,True)  # (len_time, ..., feature_dim)
            if self.load_external:
                df = self._add_external_information(df, ext_data)
            x_s, y_s = self._generate_input_data(df)
        ...
        self._logger.info("x_s shape: " + str(x_s.shape) + ", y_s shape: " + str(y_s.shape))

        #Target
        if isinstance(self.target_data_files, list):
            target_data_files = self.target_data_files.copy()
        else:  # str
            target_data_files = [self.target_data_files].copy()
        # 加载外部数据
        if self.load_external and os.path.exists(self.target_data_path + self.target_ext_file + '.ext'):  # 外部数据集
            ext_data = self._load_ext()
        else:
            ext_data = None
        x_t_list, y_t_list = [], []
        for filename in target_data_files:
            df = self._load_dyna(filename,False)  # (len_time, ..., feature_dim)
            if self.load_external:
                df = self._add_external_information(df, ext_data)
            x_t, y_t = self._generate_input_data(df)
            # x: (num_samples, input_length, ..., input_dim)
            # y: (num_samples, output_length, ..., output_dim)
            x_t_list.append(x_t)
            y_t_list.append(y_t)
        x_t = np.concatenate(x_t_list)
        y_t = np.concatenate(y_t_list)
        self._logger.info("Dataset created")
        self._logger.info("x_t shape: " + str(x_t.shape) + ", y_t shape: " + str(y_t.shape))

        return x_s, y_s,x_t,y_t

    def _split_train_val_test(self, x, y):
        """
        source domian with abundant data
    ...
    def _scarse_split_train_val_test(self, x, y):
        """
        target domian with scarce data
        划分训练集、测试集、验证集，并缓存数据集
        ...
        if self.cache_dataset:
            ...
            self._logger.info('Saved at ' + self.tgt_cache_file_name)
        return x_train, y_train, x_val, y_val, x_test, y_test
    ...
    def _generate_train_val_test(self):
        ...
        Returns:
            tuple: tuple contains:
                ...
                y_test: (num_samples, input_length, ..., feature_dim)
        """
        xs, ys,xt,yt = self._generate_data()
        x_s_train, y_s_train, x_s_val, y_s_val, x_s_test, y_s_test = self._split_train_val_test(xs, ys)
        x_t_train, y_t_train, x_t_val, y_t_val, x_t_test, y_t_test = self._scarse_split_train_val_test(xt, yt)
        return x_s_train, y_s_train, x_s_val, y_s_val, x_s_test, y_s_test,x_t_train, y_t_train, x_t_val, y_t_val, x_t_test, y_t_test
    ...
    def _load_cache_target_train_val_test(self):
        """
        加载之前缓存好的训练集、测试集、验证集
        ...
        Returns:
            tuple: tuple contains:
                ...
                y_test: (num_samples, input_length, ..., feature_dim)
        """
        self._logger.info('Loading ' + self.tgt_cache_file_name)
        cat_data = np.load(self.tgt_cache_file_name)
        x_train = cat_data['x_train']
        y_train = cat_data['y_train']
        x_test = cat_data['x_test']
        y_test = cat_data['y_test']
    ...
    def get_data(self):
        ...
        if self.data is None:
            self.data = {}
            # x_train, y_train, x_val, y_val, x_test, y_test,x_t_train, y_t_train, x_t_val, y_t_val, x_t_test, y_t_test = self._generate_train_val_test()
            if self.cache_dataset and os.path.exists(self.cache_file_name) and os.path.exists(self.tgt_cache_file_name):
                x_train, y_train, x_val, y_val, x_test, y_test = self._load_cache_train_val_test()
                x_t_train, y_t_train, x_t_val, y_t_val, x_t_test, y_t_test = self._load_cache_target_train_val_test()
            else:
        ...
        # Target
        self.feature_dim = x_t_train.shape[-1]
        self.ext_dim = self.feature_dim - self.output_dim
        self.scaler = self._get_scalar(self.scaler_type,
                                       x_t_train[..., :self.output_dim], y_t_train[..., :self.output_dim])
        self.ext_scaler = self._get_scalar(self.ext_scaler_type,
                                           x_t_train[..., self.output_dim:], y_t_train[..., self.output_dim:])
        x_t_train[..., :self.output_dim] = self.scaler.transform(x_t_train[..., :self.output_dim])
        y_t_train[..., :self.output_dim] = self.scaler.transform(y_t_train[..., :self.output_dim])
        x_t_val[..., :self.output_dim] = self.scaler.transform(x_t_val[..., :self.output_dim])
        y_t_val[..., :self.output_dim] = self.scaler.transform(y_t_val[..., :self.output_dim])
        x_t_test[..., :self.output_dim] = self.scaler.transform(x_t_test[..., :self.output_dim])
        y_t_test[..., :self.output_dim] = self.scaler.transform(y_t_test[..., :self.output_dim])
        if self.normal_external:
            x_t_train[..., self.output_dim:] = self.ext_scaler.transform(x_t_train[..., self.output_dim:])
            y_t_train[..., self.output_dim:] = self.ext_scaler.transform(y_t_train[..., self.output_dim:])
            x_t_val[..., self.output_dim:] = self.ext_scaler.transform(x_t_val[..., self.output_dim:])
            y_t_val[..., self.output_dim:] = self.ext_scaler.transform(y_t_val[..., self.output_dim:])
            x_t_test[..., self.output_dim:] = self.ext_scaler.transform(x_t_test[..., self.output_dim:])
            y_t_test[..., self.output_dim:] = self.ext_scaler.transform(y_t_test[..., self.output_dim:])
        # 把训练集的X和y聚合在一起成为list，测试集验证集同理
        # x_train/y_train: (num_samples, input_length, ..., feature_dim)
        # train_data(list): train_data[i]是一个元组，由x_train[i]和y_train[i]组成
        train_data_t = list(zip(x_t_train, y_t_train))
        eval_data_t = list(zip(x_t_val, y_t_val))
        test_data_t = list(zip(x_t_test, y_t_test))
        # 转Dataloader
        self.train_dataloader_t, self.eval_dataloader_t, self.test_dataloader_t = \
            generate_dataloader(train_data_t, eval_data_t, test_data_t, self.feature_name,
                                self.batch_size, self.num_workers, pad_with_last_sample=self.pad_with_last_sample)
        self.num_batches_t = len(self.train_dataloader_t)

        return self.train_dataloader, self.eval_dataloader, self.test_dataloader,self.train_dataloader_t, self.eval_dataloader_t, self.test_dataloader_t
