...
class TRANS1(AbstractTrafficStateModel):
    ...
    # def __init__(self, dropout=0.1, supports=None, in_dim=2, linearout_dim=12, self.nhid=32, layers=6):
        ...
        # self.end_conv = nn.Conv1d(in_channels=self.nhid, out_channels=self.linearout_dim,kernel_size=1,stride=1)
                                    # kernel_size=(1, 1))
    ...
    def calculate_loss(self, batch):
        ...
        # y_predicted[y_predicted==0.0] = 1.0
        y_true = self._scaler.inverse_transform(y_true[..., :self.output_dim])
        y_predicted = self._scaler.inverse_transform(y_predicted[..., :self.output_dim])
        return loss.masked_mae_torch(y_predicted, y_true,0.1)
