...
class DistillTrafficStateExecutor(AbstractExecutor):
    '''
    1. Train a base HIEST in the target domain
    2. Build a distill HIEST with pretrained model loaded
        1). Teacher fix all the parameters except the mapping mattrix
        2). Student finetune
    3. Measure the discrepancies betweent the Teacher and Student.(Check model)
    '''
    def __init__(self, config, model,target_model, data_feature):
        ...
        self.best_epoch_idx = self.config.get('best_epoch_idx', 0)
    ...
    def save_model_with_epoch(self, epoch):
        ...
        config['epoch'] = epoch
        model_path = self.cache_dir + '/' + self.config['model'] + '_' + self.config['source_dataset'] + '_epoch%d.tar' % epoch
        torch.save(config, model_path)
        self._logger.info("Saved model at {}".format(epoch))
        return model_path
    
    def save_tuned_model_with_epoch(self, epoch):
        """
        保存某个epoch的模型
    ...
    def tune(self, train_dataloader, eval_dataloader):
        """
        use data to train model with config
        ...
        for epoch_idx in range(self._tune_epoch_num, self.epochs):
            ...
            if val_loss < min_val_loss:
                wait = 0
                if self.saved:
                    model_file_name = self.save_tuned_model_with_epoch(epoch_idx)
                    self._logger.info('Val loss decrease from {:.4f} to {:.4f}, '
                                      'saving to {}'.format(min_val_loss, val_loss, model_file_name))
                min_val_loss = val_loss
                best_epoch = epoch_idx
